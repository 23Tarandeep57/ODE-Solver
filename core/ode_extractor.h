...
class ODEExtractor {
public:
  static void flatten_terms(const ExprPtr &e, bool is_neg,
  ...
  static ExprPtr extract_factor(const ExprPtr &term, const ExprPtr &target) {
    if (expr_compare(term, target) == 0)
      return make_num(1); // term is exactly the target variable
      ...
      for (size_t i = 0; i < mul->args.size(); ++i) {
        if (ExprPtr res = extract_factor(mul->args[i], target)) {
          ...
          for (size_t j = 0; j < mul->args.size(); ++j) {
            if (j != i)
              product = make_mul(product, mul->args[j]);
          }
          return product;
        }
      }
...
public:
  static StandardForm extract_linear_first_order(const ExprPtr &eqn,
    ...
    if (auto *na = dynamic_cast<const Number *>(A.get())) {
      if (na->value == 0) {
        throw std::runtime_error(
            "not a first-order ODE, coefficient of y' is zero");
      }
    }

    ExprPtr P = Simplifier::simplify(make_div(B, A));
    ExprPtr Q = Simplifier::simplify(make_div(make_mul(make_num(-1), C), A));
    return {P, Q};
  }
};