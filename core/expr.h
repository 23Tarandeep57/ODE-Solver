...
public:
  ...
  ExprPtr derivative(const std::string &var) const override {
    if (name == var)
      return make_num(1);
    if (depends_on == var)
      return make_derivative(make_sym(name, depends_on), var);
    return make_num(0);
  }
};
...
public:
  ...
  Apply(std::string h, std::vector<ExprPtr> a, std::string dv = "", int do_ = 0)
      : head(std::move(h)), args(std::move(a)), deriv_var(std::move(dv)),
        deriv_order(do_) {}
  ...
  int compare_same_type(const Expr *other) const override {
    ...
    if (head == "Deriv") {
      ...
      if (c)
        return c;
      if (deriv_order != o->deriv_order)
        return deriv_order < o->deriv_order ? -1 : 1;
    }
  ...
  double evaluate(const std::map<std::string, double> &env) const override {
    ...
    if (head == "Log") {
      double v = args[0]->evaluate(env);
      if (v <= 0)
        throw std::runtime_error("Log of non-positive");
      return std::log(v);
    }
    if (head == "Exp")
      return std::exp(args[0]->evaluate(env));
    if (head == "Deriv")
      throw std::runtime_error("Cannot evaluate symbolic derivative d/d" +
                               deriv_var);
  ...
  std::string to_string() const override {
    ...
    if (head == "Deriv") {
      if (deriv_order == 1)
        return "d(" + args[0]->to_string() + ")/d" + deriv_var;
      return "d^" + std::to_string(deriv_order) + "(" + args[0]->to_string() +
             ")/d" + deriv_var + "^" + std::to_string(deriv_order);
    }
    return head + "(...)";
  }
...
namespace detail {
using ApplyKey =
    std::tuple<std::string, std::vector<const Expr *>, std::string, int>;
struct ApplyKeyHash {
  size_t operator()(const ApplyKey &k) const {
    size_t h = std::hash<std::string>{}(std::get<0>(k));
    for (auto *p : std::get<1>(k))
...
inline auto &apply_pool() {
  static std::unordered_map<ApplyKey, std::weak_ptr<const Expr>, ApplyKeyHash>
      p;
  return p;
}
} // namespace detail
...
inline const Apply *as_apply(const ExprPtr &e) {
  return dynamic_cast<const Apply *>(e.get());
}
inline const Apply *as_apply(const ExprPtr &e, const std::string &head) {
  auto *a = as_apply(e);
  return (a && a->head == head) ? a : nullptr;
}

inline ExprPtr rebuild_apply(const Apply *app, std::vector<ExprPtr> new_args) {
  if (app->head == "Add") {
    ExprPtr r = new_args[0];
    for (size_t i = 1; i < new_args.size(); ++i)
      r = make_add(r, new_args[i]);
    return r;
  }
  if (app->head == "Mul") {
    ExprPtr r = new_args[0];
    for (size_t i = 1; i < new_args.size(); ++i)
      r = make_mul(r, new_args[i]);
    return r;
  }
  if (app->head == "Pow")
    return make_pow(new_args[0], new_args[1]);
  if (app->head == "Log")
    return make_log(new_args[0]);
  if (app->head == "Exp")
    return make_exp(new_args[0]);
  if (app->head == "Deriv")
    return make_derivative(new_args[0], app->deriv_var, app->deriv_order);
  return make_apply(app->head, std::move(new_args), app->deriv_var,
                    app->deriv_order);
}

inline ExprPtr map_children(const ExprPtr &e,
...
inline ExprPtr make_apply(const std::string &head, std::vector<ExprPtr> args,
  ...
  std::vector<const Expr *> kp;
  kp.reserve(args.size());
  for (auto &a : args)
    kp.push_back(a.get());
  detail::ApplyKey key{head, kp, dv, dord};
  auto &pool = detail::apply_pool();
  if (auto sp = try_pool(pool, key))
    return sp;
...
inline ExprPtr make_nary(const std::string &head, ExprPtr a, ExprPtr b,
  ...
  if (!std::isnan(absorb_check) && accum == absorb_check)
    return make_num(absorb_check);

  if (symbolic.empty())
    return make_num(accum);

  if (accum != identity)
    symbolic.push_back(make_num(accum));
...
inline ExprPtr make_add(ExprPtr a, ExprPtr b) {
  return make_nary("Add", std::move(a), std::move(b), 0.0);
}
inline ExprPtr make_mul(ExprPtr a, ExprPtr b) {
  return make_nary("Mul", std::move(a), std::move(b), 1.0, 0.0);
}

inline ExprPtr make_sub(ExprPtr a, ExprPtr b) {
  auto *na = as_num(a), *nb = as_num(b);
  if (na && nb)
    return make_num(na->value - nb->value);
  if (is_num(b, 0))
    return a;
  if (a.get() == b.get())
    return make_num(0);
...
inline ExprPtr make_pow(ExprPtr a, ExprPtr b) {
  auto *na = as_num(a), *nb = as_num(b);
  if (na && nb)
    return make_num(std::pow(na->value, nb->value));
  if (is_num(b, 0))
    return make_num(1);
  if (is_num(b, 1))
    return a;
...
inline ExprPtr make_log(ExprPtr a) {
  if (auto *n = as_num(a)) {
    if (n->value > 0)
      return make_num(std::log(n->value));
  }
  if (auto *e = as_apply(a, "Exp"))
    return e->args[0];
  return make_apply("Log", {std::move(a)});
}

inline ExprPtr reduce_exp(const ExprPtr &arg) {
  // Check if the argument is a Multiplication (e.g., c * ln(x))
  if (auto *apply_arg = dynamic_cast<const Apply *>(arg.get())) {
    if (apply_arg->head == "Mul") {

      // Assume N-ary structure: we look for a Number and a Log
      double c = 1.0;
      ExprPtr log_arg = nullptr;
      std::vector<ExprPtr> remaining_factors;

      for (const auto &factor : apply_arg->args) {
        if (auto *num = dynamic_cast<const Number *>(factor.get())) {
          c *= num->value; // Accumulate constants
        } else if (auto *factor_apply =
                       dynamic_cast<const Apply *>(factor.get())) {
          if (factor_apply->head == "Log") {
            log_arg = factor_apply->args[0]; // Extract 'u' from ln(u)
          } else {
            remaining_factors.push_back(factor);
          }
        } else {
          remaining_factors.push_back(factor);
        }
      ...
      // Execute Identity: e^(c * ln(u) * ...) -> u^c * e^(...)
      if (log_arg && remaining_factors.empty()) {
        // Exact match: e^(c * ln(u)) -> u^c
        return make_pow(log_arg, make_num(c)); // Use make_pow for consistency
      }
    }
  }
  // Check for pure Log: e^(ln(u)) -> u
  if (auto *apply_arg = as_apply(arg, "Log")) {
    return apply_arg->args[0];
  }

  // Fallback: Return standard unevaluated Exp node
  return make_apply("Exp", {arg});
}

inline ExprPtr make_exp(ExprPtr a) {
  if (auto *n = as_num(a))
    return make_num(std::exp(n->value));
  return reduce_exp(a);
}
...
inline ExprPtr make_derivative(ExprPtr target, const std::string &var,
  ...
  if (auto *d = as_apply(target, "Deriv"))
    if (d->deriv_var == var)
      return make_derivative(d->args[0], var, d->deriv_order + order);
...
inline ExprPtr Apply::derivative(const std::string &var) const {
  if (head == "Add") {
    ...
    for (size_t i = 1; i < args.size(); ++i)
      r = make_add(r, args[i]->derivative(var));
    return r;
  }
  ...
  if (head == "Pow") {
    ...
    if (n)
      return make_mul(
          make_mul(args[1], make_pow(args[0], make_num(n->value - 1))),
          args[0]->derivative(var));
    ExprPtr uv = make_pow(args[0], args[1]);
    return make_mul(
        uv, make_add(make_mul(args[1]->derivative(var), make_log(args[0])),
                     make_mul(args[1],
                              make_div(args[0]->derivative(var), args[0]))));
  }
  if (head == "Log")
    return make_div(args[0]->derivative(var), args[0]);
  if (head == "Exp")
    return make_mul(make_exp(args[0]), args[0]->derivative(var));
  if (head == "Deriv") {
    if (var == deriv_var)
      return make_derivative(
          std::make_shared<const Apply>(head, args, deriv_var, deriv_order + 1),
          var);
