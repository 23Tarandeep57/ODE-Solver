...
private:
  static ODEMetrics traverse(const ExprPtr &e, const std::string &dep_var) {
    ...
    if (auto *mul = as_apply(e, "Mul")) {
      ...
      for (const auto &arg : mul->args) {
        ...
        if (A.order != -1) {
          dep_count++;
          if (dep_count > 1)
            res.is_linear = false; // Cross-multiplication destroys linearity
    ...
    if (auto *pow = as_apply(e, "Pow")) {
      ...
      if (E.order != -1)
        return {std::max(B.order, E.order), 1, false};
      ...
      if (B.order != -1) {
        if (auto *num = dynamic_cast<const Number *>(pow->args[1].get())) {
          ...
          if (num->value < 0) {
            ...
            return {B.order, static_cast<int>(std::abs(num->value)) * B.degree,
                    false};
          }
